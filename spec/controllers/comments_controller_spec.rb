...
RSpec.describe CommentsController, type: :controller do
  describe "comments#create action" do
    it "should allow users to create comments on posts" do
      ...
      post :create, params: { post_id: post.id, comment: { message: 'test comment' } }

      expect(response).to redirect_to root_path
      expect(post.comments.length).to eq 1
      expect(post.comments.first.message).to eq "test comment"
    end

    it "should require a user to be logged in to comment on a post" do
      post = FactoryBot.create(:post)
      post :create, params: { post_id: post.id, comment: { message: 'test comment' } }
      expect(responsee).to redirect_to new_user_session_path
    end
